...
typedef OnBytesReceived = void Function(List<int> bates);
...
class TcpClient {
  ...
  Future<void> connect(String host, int port, bool useTls, SecurityContext? context, Duration? timeout) async {
    if (isOpen) {
    ...
    if (useTls) {
      try {
        stopwatch.reset();
        socket = await SecureSocket.secure(socket, context: context ?? SecurityContext.defaultContext);
    ...
    _socket.listen((bytes) {
      try {
        ...
        onError: (dynamic error, dynamic stackTrace) => {
              if (stackTrace is StackTrace) {_onClosed0(error, stackTrace)} else {_onClosed0(error, null)}
            },
        cancelOnError: true);
  }
